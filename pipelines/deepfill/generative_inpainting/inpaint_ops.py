...
def resize_mask_like(mask, x):
    ...
    """
    mask_resize = resize(
        mask, to_shape=tf.shape(x)[1:3], # Pass tensor shape dynamically (Height, Width)
...
def contextual_attention(f, b, mask=None, ksize=3, stride=1, rate=1,
    ...
    b_shape = tf.shape(b)
    b = resize(b, to_shape=tf.cast(tf.stack([b_shape[1]/rate, b_shape[2]/rate]), tf.int32), func=tf.image.resize_nearest_neighbor)
